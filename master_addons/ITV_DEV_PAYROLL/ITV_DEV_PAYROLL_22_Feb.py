...
class hr_special_allowance(osv.osv):
    ...
    _columns = {
        ...
        'position': fields.function(_get_name,arg=2,type='many2one',obj='hr.job',string='Position',),
        # 'employee_id': fields.function(_get_name,arg=3,type="char",string='Employee PIN',),
        'employee_pin': fields.selection(_populate_employee_pin, type='char', string='Employee PIN'),
        'int_note': fields.text('Note'),
    }

    def onchange_employee_pin(self,cr,uid,ids,employee_pin,context=None):
        result = {}
        if employee_pin is not False:
            emp_get=self.pool.get('hr.employee')
            id=emp_get.search(cr,uid,[('employee_pin','=',employee_pin)],context=context)
            if id:
                emp_brw=emp_get.browse(cr,uid,id[0],context=context)
                emp_name=id[0]
            else:
                emp_name=False
            result['value'] = {'emp_id': emp_name,}
            return result
        else:
            return result
    ...
    def onchange_employee_id(self,cr,uid,ids,emp_id,context=None):
        ...
        if emp_id:
           ...
           emp_search = emp_obj.search(cr, uid, [('id','=',emp_id)], context=context)
           emp_brs=emp_obj.browse(cr,uid,emp_id,context=context)

           if emp_brs:
               result['value'] = {
                   'employee_pin':emp_brs.employee_pin,
               }
           else:
               result['value'] = {
                   'employee_pin':False,
...
class hr_other_earnings(osv.osv):
    ...
    _columns = {
        ...
        'amount':fields.float('Overtime Hour',digits=(20,2),required=True,),
        'department': fields.function(_get_name,arg=1,type='many2one',obj='hr.department',string='Department',),
        'position': fields.function(_get_name,arg=2,type='many2one',obj='hr.job',string='Position',),
        # 'employee_id': fields.function(_get_name,arg=3,type="char",string='Employee PIN',),
        'employee_pin': fields.selection(_populate_employee_pin, type='char', string='Employee PIN'),
...
class hr_night_allowance(osv.osv):
    def on_change_emp_id(self, cursor, uid, ids, emp_id, context=None):
    ...
    def _get_name(self, cursor, user, ids, name, arg, context):
        print arg
        current_id = ids[0]
        cursor.execute("""SELECT emp_id FROM hr_night_allowance WHERE id=%s """
                       ,[current_id])
        row = cursor.fetchone()
        #print row
        emp_id = row[0]
    ...
    _name = "hr.night.allowance"
    _columns = {

        'emp_id':fields.many2one('hr.employee', 'Employee', required=True,),
        'date':fields.date('Date',required=True,),
        'night_no': fields.float('Number of Nights'),
        'night_rate': fields.float('Per Night Rate'),
        'amount':fields.float('Overtime Hour',digits=(20,2),required=True,),
        'department': fields.function(_get_name,arg=1,type='many2one',obj='hr.department',string='Department',),
        'position': fields.function(_get_name,arg=2,type='many2one',obj='hr.job',string='Position',),
        # 'employee_id': fields.function(_get_name,arg=3,type="char",string='Employee PIN',),
    ...
    @api.one
    @api.onchange('night_no')
    def night_allowance_computation(self):
        self.amount = self.night_no * self.night_rate
...
class hr_absent_deduction(osv.osv):
    ...
    _columns = {
        ...
        'emp_id':fields.many2one('hr.employee', 'Employee', required=True,),
        'date':fields.date('Date',required=True,),
        'amount':fields.float('Number of Days',digits=(20,2),required=True,),
        'department': fields.function(_get_name,arg=1,type='many2one',obj='hr.department',string='Department',),
        'position': fields.function(_get_name,arg=2,type='many2one',obj='hr.job',string='Position',),
        # 'employee_id': fields.function(_get_name,arg=3,type="char",string='Employee PIN',),
        'employee_pin': fields.selection(_populate_employee_pin, type='char', string='Employee PIN'),
...
class hr_loan_deduction(osv.osv):
    def _get_name(self, cursor, user, ids, name, arg, context):
        print arg
        current_id = ids[0]
        cursor.execute("""SELECT emp_id FROM hr_mobile_bill WHERE id=%s """
...
class hr_eips_deduction(osv.osv):
...
class hr_pfloan_deduction(osv.osv):
...
class hr_carloan_deduction(osv.osv):
...
class hr_holiday_allowance(osv.osv):
    ...
    def _get_name(self, cursor, user, ids, name, arg, context):
        ...
        if arg==2:
            ...
            return {current_id:job_id}
        if arg==3:
            cursor.execute("""SELECT employee_pin FROM hr_employee WHERE id=%s """
                           ,[emp_id])
            row = cursor.fetchone()
            #print row
            employee_id = row[0]
...
#maternity allowance
class hr_maternity_allowance(osv.osv):
    def on_change_emp_id(self, cursor, uid, ids, emp_id, context=None):
    ...
    def _get_name(self, cursor, user, ids, name, arg, context):
        #print arg
        current_id = ids[0]
        cursor.execute("""SELECT emp_id FROM hr_maternity_allowance WHERE id=%s """
                       ,[current_id])
        row = cursor.fetchone()
        #print row
        emp_id = row[0]
    ...
    _columns = {
        ...
        'payment_type': fields.selection([
            ('first_pmt', 'First 56days Payment'),
            ('second_pmt', 'Second 56days Payment'),
            ], 'Payment Type',),
    }
...
class hr_payroll_dev(osv.osv):
    ...
    #     date_to=row[2]
    #
    #     cursor.execute("""SELECT SUM(amount) AS totHolAlw FROM hr_holiday_allowance WHERE emp_id=%s AND date>=%s AND date<=%s """
    #                    ,[emp_id,date_from,date_to])
    #     totalHolidayAllowance = cursor.fetchone()
    #
    #     return {ids[0]:totalHolidayAllowance}

    def _get_holiday_allowance(self, cursor, user, ids, name, arg, context):
        #print 'Computing Daily Allowance'
        current_id = ids[0]
        cursor.execute("""SELECT employee_id,date_from,date_to FROM hr_payslip WHERE id=%s """
                       ,[current_id])
        ...
        date_to=row[2]
        #print date_from,' ',date_to
        cursor.execute("""SELECT SUM(amount) AS totDaiAlw FROM hr_holiday_allowance WHERE emp_id=%s AND date>=%s AND date<=%s """
                       ,[emp_id,date_from,date_to])
        totalNightAllowance = cursor.fetchone()[0]
        #print totalDailyAllowance
        return {ids[0]:totalNightAllowance}

    def _get_maternity_allowance(self, cursor, user, ids, name, arg, context):
        ...
        cursor.execute("""SELECT SUM(amount) AS totMatAlw FROM hr_maternity_allowance WHERE emp_id=%s AND date>=%s AND date<=%s """
                       ,[emp_id,date_from,date_to])
        totalMaternityAllowance = cursor.fetchone()[0]

        return {ids[0]:totalMaternityAllowance}

    def _get_night_allowance(self, cursor, user, ids, name, arg, context):
        #print 'Computing Daily Allowance'
        current_id = ids[0]
        cursor.execute("""SELECT employee_id,date_from,date_to FROM hr_payslip WHERE id=%s """
                       ,[current_id])
        ...
        date_to=row[2]
        #print date_from,' ',date_to
        cursor.execute("""SELECT SUM(amount) AS totDaiAlw FROM hr_night_allowance WHERE emp_id=%s AND date>=%s AND date<=%s """
                       ,[emp_id,date_from,date_to])
        totalNightAllowance = cursor.fetchone()[0]
        #print totalDailyAllowance
        return {ids[0]:totalNightAllowance}
    ...
    def _get_loan_deduction(self, cursor, user, ids, name, arg, context):
        #print 'Computing Daily Allowance'
        current_id = ids[0]
        cursor.execute("""SELECT employee_id,date_from,date_to FROM hr_payslip WHERE id=%s """
                       ,[current_id])
        ...
        date_to=row[2]
        #print date_from,' ',date_to
        cursor.execute("""SELECT SUM(amount) AS totDaiAlw FROM hr_loan_deduction WHERE emp_id=%s AND date>=%s AND date<=%s """
                       ,[emp_id,date_from,date_to])
        totalDailyAllowance = cursor.fetchone()[0]
        #print totalDailyAllowance
        return {ids[0]:totalDailyAllowance}

    def _get_eips_deduction(self, cursor, user, ids, name, arg, context):
        #print 'Computing Daily Allowance'
        current_id = ids[0]
        cursor.execute("""SELECT employee_id,date_from,date_to FROM hr_payslip WHERE id=%s """
                       ,[current_id])
        ...
        date_to=row[2]
        #print date_from,' ',date_to
        cursor.execute("""SELECT SUM(amount) AS totDaiAlw FROM hr_eips_deduction WHERE emp_id=%s AND date>=%s AND date<=%s """
                       ,[emp_id,date_from,date_to])
        totalDailyAllowance = cursor.fetchone()[0]
        #print totalDailyAllowance
        return {ids[0]:totalDailyAllowance}

    def _get_pfloan_deduction(self, cursor, user, ids, name, arg, context):
        #print 'Computing Daily Allowance'
        current_id = ids[0]
        cursor.execute("""SELECT employee_id,date_from,date_to FROM hr_payslip WHERE id=%s """
                       ,[current_id])
        ...
        date_to=row[2]
        #print date_from,' ',date_to
        cursor.execute("""SELECT SUM(amount) AS totDaiAlw FROM hr_pfloan_deduction WHERE emp_id=%s AND date>=%s AND date<=%s """
                       ,[emp_id,date_from,date_to])
        totalDailyAllowance = cursor.fetchone()[0]
        #print totalDailyAllowance
        return {ids[0]:totalDailyAllowance}

    def _get_carloan_deduction(self, cursor, user, ids, name, arg, context):
        #print 'Computing Daily Allowance'
        current_id = ids[0]
        cursor.execute("""SELECT employee_id,date_from,date_to FROM hr_payslip WHERE id=%s """
                       ,[current_id])
        ...
        date_to=row[2]
        #print date_from,' ',date_to
        cursor.execute("""SELECT SUM(amount) AS totDaiAlw FROM hr_carloan_deduction WHERE emp_id=%s AND date>=%s AND date<=%s """
                       ,[emp_id,date_from,date_to])
        totalDailyAllowance = cursor.fetchone()[0]
        #print totalDailyAllowance
        return {ids[0]:totalDailyAllowance}
    ...
    _columns = {
        ...
        'daily_allowance': fields.function(_get_daily_allowance,type='float',string='Attendance Allowance',store=True),
        ...
        'bonus': fields.function(_get_bonus,type='float',string='Incentive',store=True),
        'holiday_allowance':fields.function(_get_holiday_allowance,type='char',string='Holiday Allowance',store=True),
        'maternity_allowance':fields.function(_get_maternity_allowance, type='float', string='Maternity Leave Payment', store=True),
        ...
        'night_allowance': fields.function(_get_night_allowance, type='char',string='Night Allowance',store=True),
        ...
        'loan_deduction': fields.function(_get_loan_deduction,type='float',string='Loan Deduction',store=True),
        'eips_deduction': fields.function(_get_eips_deduction,type='float',string='EIPS Deduction',store=True),
        'pfloan_deduction': fields.function(_get_pfloan_deduction,type='float',string='PF Loan Deduction',store=True),
        'carloan_deduction': fields.function(_get_carloan_deduction,type='float',string='Car Loan Deduction',store=True),
    }
    _defaults = {
...
class contract_addition(osv.osv):
    ...
    _columns = {
        ...
        'provident_fund_amount': fields.float('Provident Fund Amount'),
...
class hr_salary_rule_range(osv.Model):
    ...
    _columns = {
        ...
        'leave_wto_pay':fields.boolean('Leave without Pay'),
        'absent_deduction':fields.boolean('Absent Deduction'),
